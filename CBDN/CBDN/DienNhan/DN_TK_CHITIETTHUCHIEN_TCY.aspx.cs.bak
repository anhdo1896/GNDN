...
namespace MTCSYT
{
    public partial class DN_TK_CHITIETTHUCHIEN_TCY : BasePage
    {
        ...
        private void _DataBind()
        {
            ...
            if (cmbChonDonVi.Value != null)
            {
                ...
                if (khDN.Count() > 0)
                {
                    ...
                    btnIN.Visible = true;
                    //grdDVT.DataSource = db.DN_TK_ThucTeDonVi(int.Parse(cmbChonDonVi.Value+""), int.Parse(cmbThang.Value + ""), int.Parse("" + cmbNam.Value)).ToList();
                    //grdDVT.DataBind();
                }
                else
                {
        ...
        {
            MTCSYT.SYS_Session session = (MTCSYT.SYS_Session)Session["SYS_Session"];
            Response.Redirect("../Report/Report.aspx?Thang=" + cmbThang.Value + "&Nam=" + cmbNam.Value + "&Loai=10&DonVi=" + cmbChonDonVi.Value);
