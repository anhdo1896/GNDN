...
namespace MTCSYT.Report
{
    public partial class InBienBanQT : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public InBienBanQT(DataTable dt, string thang, string nam, bool GiaoKy, bool NhanKy, string userKyGiao, string userKyNhan, string tenDonviGiao, String TenDVNhan, string strImTPNhan, string strImTPGiao, string strImGDNhan, string strImGDGiao)
        {
            ...
            xrAB_N_BT.Text = string.Format("{0:N0} ", dt.Compute("sum(Nhan_Bieu1_SanLuong)", "Loai>0 and Loai<5")).Replace(",", ".");
            xrAB_N_CD.Text = string.Format("{0:N0} ", dt.Compute("sum(Nhan_Bieu2_SanLuong)", "Loai>0 and Loai<5")).Replace(",", ".");
            xrAB_N_TD.Text = string.Format("{0:N0} ", dt.Compute("sum(Nhan_Bieu3_SanLuong)", "Loai>0 and Loai<5")).Replace(",", ".");
            xrAB_N_1Gia.Text = string.Format("{0:N0} ", dt.Compute("sum(Nhan1Gia)", "1=1")).Replace(",", ".");
            xrAB_N_TC.Text = string.Format("{0:N0} ", dt.Compute("sum(TongNhan3B)", "Loai>0 and Loai<5")).Replace(",", ".");

            xrAB_G_BT.Text = string.Format("{0:N0} ", dt.Compute("sum(Giao_Bieu1_SanLuong)", "Loai>0 and Loai<5")).Replace(",", ".");
            xrAB_G_CD.Text = string.Format("{0:N0} ", dt.Compute("sum(Giao_Bieu2_SanLuong)", "Loai>0 and Loai<5")).Replace(",", ".");
            xrAB_G_TD.Text = string.Format("{0:N0} ", dt.Compute("sum(Giao_Bieu3_SanLuong)", "Loai>0 and Loai<5")).Replace(",", ".");
            xrAB_G_1Gia.Text = string.Format("{0:N0} ", dt.Compute("sum(Giao1Gia)", "Loai>0 and Loai<5")).Replace(",", ".");
            xrAB_G_TC.Text = string.Format("{0:N0} ", dt.Compute("sum(TongGiao3B)", "Loai>0 and Loai<5")).Replace(",", ".");

            xrAB_TT_BT.Text = string.Format("{0:N0} ", dt.Compute("sum(B1_TieuThu)", "Loai>0 and Loai<5")).Replace(",", ".");
            xrAB_TT_CD.Text = string.Format("{0:N0} ", dt.Compute("sum(B2_TieuThu)", "Loai>0 and Loai<5")).Replace(",", ".");
            xrAB_TT_TD.Text = string.Format("{0:N0} ", dt.Compute("sum(B3_TieuThu)", "Loai>0 and Loai<5")).Replace(",", ".");
            xrAB_TT_1Gia.Text = string.Format("{0:N0} ", dt.Compute("sum(Tong1Gia)", "Loai>0 and Loai<5")).Replace(",", ".");
            xrAB_TT_TC.Text = string.Format("{0:N0} ", dt.Compute("sum(Tong_TieuThu)", "Loai>0 and Loai<5")).Replace(",", ".");
